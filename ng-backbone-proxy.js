...
(function(window, document, undefined) {
  ...
  /*
   ...
   @description
   Base NgBackbone model extends Backbone.model by adding additional properties and functions, including `$proxy` and `$status`. When overriding NgBackboneModel `set` method but you would like to keep `$proxy`, you'll have to explicitly call NgBackboneModel set:
   ```javascript
   ...
   The `$proxy` property allows application to use AngularJS two-way binding to manipulate Backbone objects using Backbone `get` and `set`.
   HTML:
   ```html
  ...
  factory('NgBackboneModel', ['$rootScope', 'Backbone', function($rootScope, Backbone) {
    ...
    return Backbone.Model.extend({
      ...
      /*
       ...
       @name setBinding
       @description
       Add binding on `$proxy` to a key on `attributes`
       */
